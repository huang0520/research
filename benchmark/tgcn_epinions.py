...
from benchmark.epinions import no_reduce, reduce_both, reduce_edge
...
normal = ModelConfig(
    name="TGCN_Normal",
    model_fn=lambda _: TGCN(2, 32, 100, gcn_norm=False, cache_gconv=False).to("cuda"),
)

cached = ModelConfig(
    name="TGCN_Cached",
    model_fn=lambda max_nodes: TGCN(
        2, 32, 100, gcn_norm=False, cache_gconv=True, max_nodes=max_nodes
    ).to("cuda"),
)
